...
SQLite or SQLCipher, set up to read and write SQLCipher's format. This repository uses it for the native tests: the
encryption layer of SQLite3 Multiple Ciphers sits on top of the VFS under test, as it does in wasm builds through
`sqlite-wasm-rs`.

Apart from this directory, the crate is the one published on crates.io, whose `.crate` file has the SHA-256
`95b4103cffefa72eb8428cb6b47d6627161e51c2739fc5e3b734584157bc642a`. The changes are:

- the `bundled-sqlite3mc` feature in `../Cargo.toml` and `../Cargo.toml.orig`, and its handling in `../build.rs`;
- the bundled SQLite and SQLCipher sources are left out (`../sqlite3/` except `wasm32-wasi-vfs.c`, which `../build.rs`
  names in a `rerun-if-changed`, and `../sqlcipher/`). Only the `bundled-sqlite3mc` feature builds from this copy.

## Source

- SQLite3 Multiple Ciphers 2.4.0, based on SQLite 3.53.4. For comparison, `sqlite-wasm-rs` 0.5.5 bundles 2.3.3.
...
- `LICENSE` is the MIT license of SQLite3 Multiple Ciphers, from the `v2.4.0` tag of its repository; the zip doesn't
  include it.
- `bindgen_bundled_version.rs` and `bindgen_bundled_version_ext.rs` are copies of the bindings that libsqlite3-sys
  0.36.0 ships in its `sqlite3/` directory, generated for SQLite 3.51.1. SQLite's C API only grows, so they cover a subset of 3.53.4. For the same reason,
...
1. Keep the version close to the one `sqlite-wasm-rs` bundles for wasm builds.
